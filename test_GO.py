...
def test_GO_relation_2():
    ...
    assert relation.id == 'rel'
    assert relation.name == 'relation'
    assert relation.is_transitive is True
    assert relation.others == {}
    assert str(relation) == '<rel>'
...
def test_GO_4():
    go = GO.GO('go.obo')
    assert len(go.relations['is_a_related_to_part_of'][go.categories['GO:0005829']]) == 2
